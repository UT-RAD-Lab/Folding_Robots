...
//double response = 0;
//const double gyroWeight = 0.95;
//const double expoWeight = 0.8;
...
void loop() {
  ...
  /*
    //Complimentary Filter + Exponential Filter
    pitch += (double(input[5]) / 131) * dt;
    int accMag = abs(input[0]) + abs(input[1]) + abs(input[2]);
    if (accMag > 8192 && accMag < 32768) {
    pitchAcc = double(atan2(input[0], -input[2])) * 180 / PI + 91.5;
    pitch = gyroWeight * pitch + (1 - gyroWeight) * pitchAcc;
    }
    //pitchFilt = expoWeight * pitch + (1 - expoWeight) * pitchPrev;
    pitchOffset = atan((sin(response)) / (3 + cos(response)));
    //response = constrain(map(-1.75 * (pitch - pitchOffset), -120, 120, Min[2], Max[2]), Min[2], Max[2]);
    response = constrain(map((pitch - pitchOffset), -120, 120, Min[2], Max[2]), Min[2], Max[2]);


    Serial.println(String(pitchAcc) + '\t' + String(pitch));
    angle[2].write(response);
    pitchPrev = pitch;
    delay(5);*/
}
...
void measure() {
  ...
  //data[1][0] = map(input[0], -32768, 32768, 0, 200);
  //data[1][1] = map(input[2], -32768, 32768, 0, 200);
  //data[1][2] = map(-input[5], -32768, 32768, 0, 200);
